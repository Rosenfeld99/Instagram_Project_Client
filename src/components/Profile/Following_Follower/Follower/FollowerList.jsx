...
const FollowerList = () => {
  ...
  const getFollowersList = async () => {
    try {
      const _url = USER_ROUTE + "followers/" + userName;
      const { data } = await apiGet(_url);
      console.log(data);
      setDataList(data.followers);
      setFilterData(data.followers);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const center = <p className=" font-semibold">Followers</p>;
  return (
    <div>
      <div>
        <NavTopMobile
            ...
            <div className=" w-full">
              {filterData?.length > 0 ? (
                <div className=" flex items-center justify-center flex-col gap-5">
                  {filterData?.map((item) => (
                    <FollowerItem item={item} key={item?._id} />
                  ))}
                </div>
              ) : (
                <div>
                  {inputValue === "" && dataList?.length > 0 ? (
